...
export async function criarMarca(req: Request, res: Response) {
  const { nome, foto } = req.body;

  try {
    const marca = await MarcasService.criarMarca(nome, foto);
...
export async function atualizarMarca(req: Request, res: Response) {
  const { id } = req.params;
  const { nome, foto } = req.body;

  try {
    const marca = await MarcasService.atualizarMarca(Number(id), nome, foto);
    res.status(200).json(marca);
  } catch (error: any) {
    res.status(error.status ?? 500).json({ erro: error.mensagem ?? "Erro interno do servidor" });
  }
